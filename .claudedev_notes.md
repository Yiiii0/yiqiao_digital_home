...
│   │   ├── Experience.astro.bak # 历史备份文件（当前不使用）
...
│   └── add-rambling-and-push.sh  # 历史脚本（参数与当前 add-rambling.js 不一致）
...
- 数据结构:
  ```json
  {
    ...
    "date": "YYYY-MM-DD",
    "content": { "en": "...", "zh": "..." } | "...",
    "tag": "AI" | "Personal" | "Event" | "..." | ["AI", "Event"]
  }
  ```
...
- 数据结构:
  ```json
  {
    ...
    "title": { "en": "...", "zh": "..." },
    "summary": { "en": "...", "zh": "..." },
    "file": { "en": "filename.en.md", "zh": "filename.md" } | "filename.md",
    "tag": "AI" | "Personal" | "Event" | "..." | ["AI", "Event"]
  }
  ```
...
- Tag 筛选: All Tags + `rambling.json` 中出现过的所有 tag（动态生成）
- 两个筛选器可以联合使用

**添加新内容:**
```bash
# 添加 snippet
node scripts/add-rambling.js snippet "content" "tag" [date]

# 添加 blog (需要先创建 markdown 文件)
# 1. 在 src/content/blogs/ 创建 your-blog.md
# 2. 运行:
node scripts/add-rambling.js blog "title" "summary" "your-blog.md" "tag" [date]
```

**当前脚本状态说明:**
- `scripts/add-rambling.js` 仍按“单语言字符串”写入（legacy 兼容），不是双语对象写入
- 若要保持双语质量，建议直接手动编辑 `src/data/rambling.json`
- `scripts/add-rambling-and-push.sh` 目前参数调用方式已过时，不建议直接使用
...
**注意:** 并非所有字段都是双语对象。当前常见模式是：
- `aboutMe` / `title` / `bullets` / `achievements` 使用 `{ en, zh }`
- `company` / `school` / `dateRange` 等字段仍可为字符串
...
当前 `Rambling.astro` 使用 `marked.js`（CDN）+ `fallbackMarkdownRenderer` 双重方案。fallback 解析器支持:
- 标题 (`#`, `##`, `###`)
- 加粗 (`**text**`)
- 斜体 (`*text*`)
- 列表 (`- item`)
...
如需更多 markdown 特性，优先扩展 `marked.js` 配置；离线兜底逻辑在 `fallbackMarkdownRenderer`。
...
# 注意: 当前 .gitignore 没有忽略 .claudedev_notes.md，提交前需手动确认
```
...
- Consolidated part of markdown rendering logic and helper usage
...
**Current State Note (2026-02-16):**
- `public/CNAME` is not present in the current repo snapshot; custom domain may be managed directly in GitHub Pages settings.

## Recent Updates (2026-02-16)

### Rambling Snippet + Notes Sync

**Change:**
- Added a new `Personal` snippet in `src/data/rambling.json` with bilingual content (`en`/`zh`)
- Theme: execution and ideas are often driven first by love and strong desire
- Synced `.claudedev_notes.md` with current codebase:
  - Updated Rambling schema/docs to match real data shape
  - Corrected tag/filter behavior description
  - Corrected markdown parser function description
  - Corrected `.gitignore` note
  - Marked legacy script status

**Modified Files:**
- `src/data/rambling.json` - Added new snippet entry (date: `2026-02-16`)
- `.claudedev_notes.md` - Documentation cleanup and sync
...
- 检查需要双语的字段（如 `title`、`summary`、`content`）是否包含 `en` / `zh`
